...
const HostelsPage = () => {
  useEffect(() => {
    // Remove any old script if it exists
    const existingScript = document.getElementById("hotellook-widget");
    if (existingScript) {
      existingScript.remove();
    }

    // Pre-filtered hostel deep link (encoded for widget)
    const hostelSearchUrl = encodeURIComponent(
      "https://search.hotellook.com/?f%5Bproperty_types%5D=7"
    );

    // Build widget script with hostel filter
    const script = document.createElement("script");
    script.src = `https://tpwdg.com/content?currency=usd&trs=446991&shmarker=661841&show_hotels=true&powered_by=true&locale=en&searchUrl=${hostelSearchUrl}&primary_override=%23FF8E01&color_button=%23FF8E01&color_icons=%23FF8E01&secondary=%23FFFFFF&dark=%23262626&light=%23FFFFFF&special=%23C4C4C4&color_focused=%23FF8E01&border_radius=5&plain=false&promo_id=7873&campaign_id=101`;
    script.async = true;
